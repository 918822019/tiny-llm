...
| eagle3-batch-verify-ablation（Android / PLK110） | 本次 | — | — | — | 无 batch 投机 **0.741×**；batch/tile 在投机内 **1.799×**；生产配对 decode **1.239×**、请求内总耗时 **1.150×** | 新增逐 token target verify 诊断和 backend 权重预准备；相同 82 个 target 输入、22/41 接受率下 sequential 82 calls、batched 41 calls，全部输出一致。7 轮仅跑 greedy/batched、相邻且交替顺序，decode 每轮均为 1.237–1.246×；旧 1.406× 混入懒上传与四路径热状态，已降级为历史值。通用 Vulkan 绝对性能仍远慢于 CPU |
...
### B-2 层内异步预取：decode 360→308 ms/tok（1.17×，累计 29.7×）（2026-09-07）

- **优化栈**：… + GPTQ NEON + embed 卸载 + 保留源 fp16 + B-1 单次 pread
  + **B-2 层内异步预取**
- **是什么**：`topk_softmax` 之后本层全部 k=8 个专家 id 已知，一次性入队预取，
  使后台 pread 与 `expert_ffn` 计算重叠。预取用**独立缓冲池**而非复用 LRU 槽
  （同时规避线程安全与指针生命周期两个风险）。
- **假设**：B-1 后计算（54.9%）比 I/O（45.1%）慢，藏住 I/O 更划算 → 预估上限
  **1.83×**
- **结果**：**1.17×**（360 → 308 ms/tok），I/O 2.61s → 1.62s（-38%）。
  generated_ids 与预取关完全一致。**累计 vs ref 基线：9157 → 308 = 29.7×**
- **验证**：190 单测全过；`[moe] prefetch: hits=464 (7%) waits=1589 (24%)
  fallbacks=4691 (70%)`
- **意外 / 教训**：
  1. **实测 1.17× 远低于预估 1.83×，根因是粒度错配**。预估按"整 token 级"算
     `max(I/O, 计算)`，但实际重叠发生在**单专家级**：

     | 单专家 | 耗时 |
     |---|---|
     | I/O（2.5 MB ÷ 5 GB/s） | **0.50 ms** |
     | 计算（ffn 1.94s ÷ 5760） | 0.34 ms |

     **I/O > 计算**，与预估假设相反。预取只能把 I/O 提前、不能消除它。
     **教训：估算 overlap 收益要在实际重叠的粒度上比较，不能在上层粒度算。**
  2. **两个死锁，都是实测踩到的**（用 `sample <pid>` 抓调用栈定位，两个线程
     都停在 `__psynch_cvwait`）：
     - **单 CV + notify_one 唤醒错误等待者**：主线程等"某 key 就绪"、预取线程
       等"队列有活"，共用一个 CV 时 `notify_one` 可能唤醒主线程，它重查条件
       不满足又睡回去，预取线程永不被唤醒 → 双方永久互等。拆成两个 CV。
     - **槽选择覆盖在飞槽**：所有槽占用后 `pf_pick_slot_locked` 永远返回 slot 0
       （key 从不清除），预取互相覆盖；主线程醒来重查自己的 key 已被覆盖 →
       nullptr → 条件假 → 睡回去 → 队列空后预取线程也睡 → 永等。修法：绝不选
       在飞槽（空闲→已就绪，全在飞则放弃本次预取），且等待条件把"槽消失"也
       算作满足（回退同步路径而非继续等）。
     **并发代码的两条铁律**：① 不同等待条件必须用不同 CV ② 等待条件必须把
     "等待对象消失"也算作满足，否则对象被复用时就永等。
  3. **decode 已接近带宽下限**：每 token 读 1.682 GB，有效带宽 **5.46 GB/s
     （M4 NVMe 峰值 84%）**，理论下限 259 ms/tok，距当前仅 **49 ms** 空间。
     要再快只能**读更少数据**（专家已是 i4 GPTQ，压缩空间有限）。
     → **CPU/IO 侧优化基本到顶，下一阶段应转向别的方向**（见下）。

- **复现**：
  ```bash
  ./scripts/verify.sh                                   # 190 单测
  # A/B（3 次取 min，见坑 #7）；预取统计看 [moe] prefetch 行
  for extra in "" "--moe-prefetch"; do
    ./build/runtime/tinyqwen --model model_qwen3_30b_moe_i4_fp16.tqwen \
      --tokens 9707,11,358,1229,9826,105129,12 --max-new-tokens 8 --max-seq-len 32 \
      --moe-ssd --moe-expert-cache-slots 4 --matvec-impl neon $extra
  done
  # 死锁定位：后台跑 + sample <pid> 1 抓调用栈
  ```

- **下一阶段方向**（CPU/IO 已到顶）：
  - **专家层多线程并行**：top-8 专家 FFN 彼此独立，48 次 fork-join/token
    （而非 kernel 级的 1152 次）。但当前 I/O 占 32.8%，计算并行收益有限。
  - **读更少数据**：更激进的专家量化（i4 → i3/i2），或按热度只常驻高频专家。
  - **B-3 投机跨 token 预取**：需先测相邻 token 专家重叠率。当前 fallbacks 70%
    说明预取本身就没喂饱，投机预取的边际收益可能很小。
...
### GDN 投影优化：隔离微基准证伪——已达带宽墙，无余量（2026-09-08，macOS M4）

- **背景**：Qwen3.5-35B-A3B 归因显示 GDN 投影占 35.6%（`gdn_proj` 27.7% +
  `gdn_out_proj` 7.9%），是最大单项（30/40 层是 Gated DeltaNet），且权重 resident
  不读盘，看起来像"纯计算优化"的好目标。
- **方法**：按坑 #11「对组件下结论前必须隔离测量它，不能从端到端时间倒推」，
  用 `bench_kernels` 单独测 GDN 主导投影，避开端到端计时污染。
- **结果**（f16 `in_proj_qkv [8192, 2048]`，权重 32.0 MB）：

  | 实现 | 耗时 | 有效带宽 | vs ref |
  |---|---|---|---|
  | ref | 14.109 ms | 2.38 GB/s | 1.00× |
  | neon_mt_kv_nt | **0.364 ms** | **92.36 GB/s** | 38.79× |

- **证伪**：M4 整机墙 ≈ **114 GB/s**（`benchmarks/machine_ceiling/m4_ceiling.md`，
  规格的 95%），92.36 GB/s 已是墙的 **81%**。所以 GDN 投影是**带宽瓶颈而非算力
  瓶颈** —— 写新 kernel 不可能有收益，限制在于读那 32 MB 权重本身。
  剩余 ~19% 余量需要完美调度才能拿到，投入产出比极低。
- **唯一可行路径**：按坑 #29 原则「要再快只能**读更少数据**」—— 量化 GDN 投影
  （当前 f16）。这与专家权重的 GPTQ 化是同一类优化，但 GDN 投影是 resident，
  量化后直接省内存带宽而非 I/O。
- **顺带的方法论教训**：这台机器 swap 常年 6–8 GB / 8 GB、wired 8143 MB（一半
  物理内存，杀进程无法释放），可用仅 1500 MB 而 35B 需 resident 3703 MB。
  **端到端计时在此条件下不可用于归因**（坑 #7/#20/#26）。但隔离微基准只需
  几 MB 内存，不受影响 —— 所以「组件级结论走隔离基准」不只是精度问题，
  在本机还是**唯一能做的测量**。

```bash
./build/benchmarks/bench_kernels --family f16 --out 8192 --in 2048
# 对照机器墙
cat benchmarks/machine_ceiling/m4_ceiling.md
```
...
- **是什么**：给 EAGLE3 新增 `--no-eagle3-batch-verify`。默认一次验证
  ...
  `target_verify_mode`。
- **测量修正**：batched 的 `N>1` tied lm_head 走 Vulkan，而 greedy/sequential 的
  `N=1` lm_head 走 CPU。旧路径在首个 speculative block 内才创建/复制约 296.8 MiB
  GPU buffer，另一个预热进程又无法保留它，导致一次性准备成本混进 decode。新增
  `IBackend::prepare_weight` / `QwenModel::prepare_lm_head`，在 EAGLE target prefill 后
  显式准备；成本移出 decode、但仍计入该请求 prefill。测试顺序也改为让
  greedy↔batched、batched↔sequential 相邻并逐轮反转。
- **测量工具**：`scripts/bench_eagle3_ablation_android.sh` 自动部署、逐轮比较完整
  `generated_ids`，输出 prefill/decode/total、同轮加速比分布和 JSON。默认 `full` 跑
  四路径；`EAGLE3_BENCH_MODE=pair` 只跑生产 greedy/batched，用于更多轮的低干扰复测。
...
- **控制变量**：sequential 与 batched 都验证 82 个 target 输入，proposal/accepted 均为
  41/22（53.66%）；sequential/batched 的物理 target 调用分别为 82/41。64 个输出
  token 在四路径及全部正式样本中完全相同。
- **完整阶梯（三轮）**：decode 中位依次为 CPU greedy 2605.78、Vulkan greedy
  8429.86、EAGLE sequential 11263.70、EAGLE batched 6573.34 ms；请求内
  prefill+decode 中位依次为 3665.35、9866.40、13190.54、8421.69 ms。按同轮比值
  取中位：投机但不 batch 为 **0.741×**，投机场景内 batch/tile 为 **1.799×**，
  完整投机 vs Vulkan greedy 的 decode 为 **1.292×**、请求内总耗时为 **1.183×**。
- **生产路径复测（七轮）**：排除 CPU/sequential 热干扰，只让 Vulkan greedy 与
  batched EAGLE 相邻并交替先后顺序。decode 中位 8251.62→6646.20 ms，逐轮加速
  **1.237–1.246×**、中位 **1.239×**；prefill+decode 中位 10003.54→8708.91 ms，
  逐轮 **1.143–1.154×**、中位 **1.150×**。另一次独立七轮配对得到 decode 中位
  1.241×，方向和量级一致。EAGLE draft/target verify 中位分别为 571.11/6077.74 ms。
- **历史数字修订**：旧四路径三轮曾得到 **1.406×**，但同时存在 lazy lm_head 准备、
  非相邻主要对照和明显热状态漂移。该数值保留作历史记录，不再作为权威性能结论；与
  llama.cpp 单轮约 1.41× 接近只能说明方向相容，不能证明稳定倍率相同。
- **机制证据**：sequential→batched 保持输入数、proposal、接受率不变，却把物理 target
  调用 82→41；无 batch 的投机只有 0.741×，排除了“drafter 自己就是加速来源”的解释。
  批量路径的两行 lm_head 走 backend matmul，Transformer 权重也在 token tile 内复用。
- **判定**：权威倍率为约 **1.24× decode / 1.15× 请求内端到端**，来源是 EAGLE
  有效提案与 token tile 的乘法交互。单有 drafter 会因额外 target 输入和 draft 成本
  倒退；单有 greedy 又没有未来 token 可填 tile。通用逐算子 Vulkan 的绝对速度仍远慢
  于 CPU，queue wait 占大头，因此不代表 TinyLLM GPU 已成为手机最佳后端，也不能等同
  于服务器论文吞吐。
- **额外扫描**：sky prompt 的 width=2/3/4 接受率为 53.7%/38.0%/29.3%，target 输入
  82/107/133，说明宽度增大后浪费迅速上升；Fibonacci/TCP width=2 接受率为
  65.8%/63.2%。这些额外性能轮次在测量修正前完成，仅将接受统计和趋势作为有效证据。
- **验证**：host Release build/CTest 0 failed；PLK110 Android **209 tests, 0 failed**；
  host sequential/batched 输出相同，且同为 53 个输入时 calls 分别为 53/27；真机四路径
  与七轮 pair 的全部输出完全一致。完整表与命令见 `docs/eagle3.md`。
