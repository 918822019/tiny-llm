...
| dtype               | u32      | 全部 tensor 的默认 dtype：0=f32（v1 仅支持 0），1=f16，2=i8（保留），3=i4（已支持，见下） |
...
matvec reference 直接按行点积。INT4 走专用 packed layout（见下节），不复用 f32 路径。

### INT4 packing（dtype=3，v2）

- **量化算法**：exporter `tools/export_qwen_to_tiny_i4.py --method hqq|rtn`。
  默认 **HQQ**（半二次优化，MSE 最优、免校准，同层 MSE 比 RTN 低 ~28%）；
  rtn = min-max 舍入，保留作回归对照。
- **粒度**：非对称 uint4 [0,15]，per-group scale+zero。group_size 存 v2 header
  `quant_group_size`（典型 64，须 ≥32 且为 32 的倍数——NEON kernel 约束）。
- **每组内存布局**（group_size=64 → 36 字节）：
  `[scale_fp16(2B) | zero_fp16(2B) | packed_uint4(group_size/2 B)]`，
  低 nibble 在前（偶数下标元素占低 4 bit）。
- **反量化**：`(q - zero) × scale`（fp16→fp32），与 HQQ 库语义一致。
- **混合 dtype**：只有 2D matmul 权重量化；embed_tokens（tied，兼 lm_head）/
  norm / bias 保留 fp32。注意后果：**tied lm_head 每 token 544MB fp32 流量**，
  是当前 INT4 路径的最大单项流量（÷4 红利被吃掉一半多，见优化日志
  i4-hqq-android 小节的 PMU 流量账）。
