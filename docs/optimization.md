...
- **常驻池的同进程 A/B 失真**：mt 系实现各自带常驻自旋线程池，同一进程里
  先后测多个 mt 实现时，闲置池的自旋线程会挤占核心——实测 q_proj 上误差
  达 2×（0.013 vs 0.025ms）。对比 mt 变体必须**分进程**跑：
  `bench_kernels --impls ref,<单一实现>`，每个实现一个进程。
