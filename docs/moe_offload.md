...
`topk_softmax` 之后本层全部 k=8 个专家 id 已知，一次性入队预取，使后台 pread
与 `expert_ffn` 计算重叠。实测：

| | 预取关 | **预取开** | 加速 |
|---|---|---|---|
| decode | 360 ms/tok | **308 ms/tok** | **1.17×** |
| I/O | 2.61s (44.2%) | **1.62s (32.8%)** | **-38%** |
| expert_ffn | 1.89s | 1.94s | 持平 |

**累计 vs ref 基线：9157 → 308 ms/tok = 29.7×**

**实测 1.17× 远低于预估上限 1.83×，根因是粒度错配。** 预估按"整 token 级"算
`max(I/O, 计算)`，但重叠实际发生在**单专家级**：
...
实测命中率 `hits 7% / waits 24% / fallbacks 70%` 就是证据。

> **教训**：估算 overlap 收益要在**实际重叠的粒度**上比较，不能在上层粒度算。

**设计决策：预取用独立缓冲池，不复用 LRU 槽**，同时规避两个风险：
1. **线程安全** —— 预取线程只碰 `pf_slots_`/队列（mutex 保护），`slots_`/
   `slot_index_` 仍只有主线程碰，无需给 LRU 加锁
2. **指针生命周期** —— 预取槽是专用缓冲、不进 LRU，不会被淘汰。B-1 后三个
   指针指向同一 `span` 缓冲，若复用 LRU 槽则淘汰即全部失效

内存代价 8 槽 × 2.4 MB = 19 MB，已计入启动预算校验。

**踩到两个死锁**（用 `sample <pid> 1` 抓调用栈定位，两个线程都停在
`__psynch_cvwait`）—— 见 AGENTS.md 坑 #27：
1. **单 CV + notify_one 唤醒错误等待者**：主线程等"某 key 就绪"、预取线程等
   "队列有活"，共用一个 CV 时 `notify_one` 可能唤醒主线程，它重查条件不满足
   又睡回去，预取线程永不被唤醒 → 双方永久互等。拆成 `pf_work_cv_` /
   `pf_ready_cv_`。
2. **槽选择覆盖在飞槽**：所有槽占用后永远返回 slot 0（key 从不清除），预取
   互相覆盖；主线程醒来重查自己的 key 已被覆盖 → nullptr → 条件假 → 睡回去
   → 队列空后预取线程也睡 → 永等。修法：绝不选在飞槽（空闲→已就绪，全在飞
   则放弃本次预取），且等待条件把"槽消失"也算作满足。

#### decode 已达带宽下限 —— CPU/IO 侧优化到顶

**四层瓶颈分析**（decode 319 ms/tok = 3.13 tok/s，预取开，3 次取 min）：

**① 时间去哪了**

| 算子 | 占比 | 每 token | 类别 |
|---|---|---|---|
| expert_load (SSD pread) | 37.4% | 119 ms | I/O |
| expert_ffn (专家 GEMM) | 32.1% | 102 ms | 计算 |
| qkv_proj | 13.6% | 43 ms | 计算 |
| o_proj | 12.4% | 40 ms | 计算 |
| lm_head | 3.3% | 11 ms | 计算 |

I/O 合计 37.4%，计算合计 62.6%。
...
| 来源 | 字节 | 占比 | 构成 |
|---|---|---|---|
| 专家（SSD） | 0.96 GB | 47% | 48 层 × 8 专家 × 2.39 MB |
| lm_head（RAM） | 0.62 GB | 30% | 151936 × 2048 × fp16 |
| qkv+o_proj（RAM） | 0.48 GB | 23% | 48 层 × 9.56 MB |
| **合计** | **2.07 GB/token** | | |

decode 是 batch=1 的 matvec：**每个权重元素只用一次** → 算术强度 1 MAC/字节。
i4 权重 0.5 字节/权重 → 2 MAC/字节。带宽上限决定的吞吐 = 6.5 GB/s × 2 =
**13 GMAC/s**，实测计算吞吐 **12.90 GMAC/s = 该上限的 99%**。

→ **decode 本质是内存带宽受限，不是计算受限。** NEON 算力只用 7.3%，但算力
不是瓶颈 —— **继续优化 kernel 不会有收益**。

**③ 为什么缓存救不了**

实测 `hits=0 / misses=7338 / evictions=7334` → **命中率零**。相邻 token 选中的
专家几乎不重叠，**无时间局部性可利用**。这也解释了为何增大 cache slots 反而
更慢（AGENTS.md 坑 #20）。
...
| 方案 | 省 | 约省时间 | 风险 |
|---|---|---|---|
| 专家 i4→i2 | 0.48 GB | 98 ms | 需精度验证 |
| lm_head fp16→i4 | 0.47 GB | 95 ms | **argmax 一致率仅 75%，不安全** |
| qkv+o_proj i4→i2 | 0.24 GB | 49 ms | 需精度验证 |

三项全做（若精度可接受）：319 → 约 77 ms/tok（**4.14×**）。

> **lm_head 量化的精度陷阱**：RTN i4 的 CosSim 是 0.995（看起来"很好"），但
> **top-1 argmax 一致率只有 75%** —— greedy decode 每 4 个 token 就有 1 个分叉。
> **量化收益必须用 argmax 一致率衡量，不能只看 CosSim**（坑 #25）。

#### lm_head 量化实测：i4 不可行，i8 是可行的中间档

实测（200 个随机 hidden 向量，对比 fp16 基线的 argmax）：

| 方案 | 大小 | 省下 | 权重 RMS 误差 | **argmax 一致率** | CosSim |
|---|---|---|---|---|---|
| fp16 基线 | 594 MB | — | 0% | 100% | 1.000000 |
| **i8 gs=128** | 306 MB | 287 MB | 0.588% | **97.5%** | 0.999982 |
| i8 gs=64 | 315 MB | 278 MB | 0.528% | **98.0%** | 0.999986 |
| RTN i4 gs=128 | 158 MB | 436 MB | 9.989% | **75.5%** | 0.994973 |
| HQQ i4 gs=128 | 158 MB | 436 MB | 9.759% | **74.5%** | 0.995195 |

**关键结论**：
1. **i4 不可行**，且**换 HQQ 也没用**（74.5% vs RTN 75.5%）。根因不是量化算法，
   是 **i4 本身对 lm_head 太粗** —— ~10% 权重误差是 i4 的理论极限
   （step/√12 ÷ 权重 RMS ≈ 11.5%），换算法只能微调，无法跨越。
2. **i8 是可行的中间档**：argmax 一致率 97.5%，权重误差 0.588%（比 i4 小 17×）。
   省 287 MB → 约 46 ms → **decode 319 → 273 ms/tok（1.17×）**。
   代价：2.5% 的 token 会分叉（约每 40 个 token 1 个）。
3. **i8 需要新的 kernel 支持**：仓库现有 dtype 是 f32/f16/i4/vq2/gptq4，
   **没有 i8**。实现需新增 dtype 编号 + i8 matvec kernel（NEON）+ exporter +
   loader 支持。工作量不小，换 1.17× 且带 2.5% 精度损失 —— 需权衡。

#### B-2 预取的净收益与代价

严格 A/B（3 次取 min）：预取 ON **319** ms/tok vs OFF **365** ms/tok = **1.14×**。
但 bytes_read **37.0 GB vs 21.2 GB（1.75×）** —— 预取线程与主线程 fallback 会
读同一专家两次。已在 fallback 时从队列移除 key（39.6 → 37.0 GB），但只能防
未来的读、防不了在飞的。彻底修复需共享"已认领"集合让预取线程 pread 前检查。

多余的是**读**（不消耗 SSD 寿命，只占带宽），且 1.14× 仍为正收益，故保留。

#### B-3：投机跨 token 预取 —— **边际收益可能很小**

专家选择在相邻 token 间常有重叠。可在层 i-1 的 FFN 期间，按**上一个 token 的
选择**投机预取层 i 的专家；猜错则回退到正常 pread（正确性不受影响）。

但 B-2 的 `fallbacks 70%` 说明**预取本身就没喂饱**（单专家 I/O 比计算慢，追不
上），投机预取的边际收益可能很小。**先测相邻 token 专家重叠率再决定是否投入。**
