...
TEST(attention_single_position) {
  // One cached position: softmax over a single score is 1, so out == v0.
  const float q[2] = {1.0f, 0.0f};
  const float k[2] = {2.0f, 0.0f};   // layout [kv=1][max=2][hd=2], pos 0 only
...
TEST(attention_gqa_shares_kv_head) {
  // Two q heads, one kv head: both heads attend to the same K/V.
...
TEST(attention_respects_max_seq_len_stride) {
  // Garbage in unused slots must never be read: this catches stride bugs
  // between [n_kv_heads][max_seq_len][head_dim] and seq_len.
  ...
  k[0] = 1; k[1] = 0; v[0] = 1; v[1] = 0;
  k[2] = 0; k[3] = 1; v[2] = 0; v[3] = 1;
...
TEST(attention_matches_softmax_kernel) {
  // Cross-check against explicit score + softmax_ref for a small random case.
