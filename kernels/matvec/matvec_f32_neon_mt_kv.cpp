// ============================================================================
// matvec_f32_neon_mt_kv.cpp — 矩阵乘向量：y = W @ x
//                  NEON + 多线程 + k/v 成对融合版
// ============================================================================
...
namespace tinyqwen {
  namespace {
    ...
    inline float dot_row_neon(const float *row, const float *x, int n) {
      ...
      const int n4 = n & ~3; // 向量尾段边界
      for (; i < n4; i += 4) {
        acc0 = vfmaq_f32(acc0, vld1q_f32(row + i), vld1q_f32(x + i));
      }
      const float32x4_t sum01 = vaddq_f32(acc0, acc1); // 合并链 0+1
      const float32x4_t sum23 = vaddq_f32(acc2, acc3); // 合并链 2+3
      float total = vaddvq_f32(vaddq_f32(sum01, sum23)); // 横向归约
      for (; i < n; ++i) { // 标量尾段
        total += row[i] * x[i];
      }
      return total;
    ...
    // ========================================================================
    // RowPool — 常驻行切分线程池（neon_mt 的 RowPool + 成对任务支持）
    // ========================================================================
    struct RowPool {
      // 任务参数：pair job 时 (w, y) 是第一个矩阵、(w2, y2) 第二个；
      // 单矩阵 job 时 w2 = nullptr
      const float *w = nullptr;   // 第一个权重矩阵
      const float *w2 = nullptr;  // 第二个权重矩阵（pair 模式）
      const float *x = nullptr;   // 共享输入向量
      float *y = nullptr;         // 第一个输出
      float *y2 = nullptr;        // 第二个输出（pair 模式）
      int out_dim = 0;            // 单个矩阵的行数；pair job 的总行数 = 2*out_dim
      int in_dim = 0;             // 输入维度
      bool pair = false;          // 是否是成对模式
      ...
      void do_chunk(int idx) const {
        ...
        const int total = pair ? 2 * out_dim : out_dim; // 总行数
        const int base = total / p;  // 基础行数
        const int rem = total % p;   // 余数
        const int begin = idx * base + (idx < rem ? idx : rem);
        const int end = begin + base + (idx < rem ? 1 : 0);
        for (int r = begin; r < end; ++r) {
          // 根据行号判断属于哪个矩阵
          const int o = pair && r >= out_dim ? r - out_dim : r; // 矩阵内行偏移
          const float *wm = pair && r >= out_dim ? w2 : w;      // 选择权重矩阵
          float *ym = pair && r >= out_dim ? y2 : y;            // 选择输出向量
          const float *row = wm + static_cast<size_t>(o) * in_dim; // 定位行起点
          ym[o] = dot_row_neon(row, x, in_dim); // 计算点积
        }
      }

      // fork-join 发布（仅 master 调用）
      void publish_and_run() {
        job_gen.store(++job_counter, std::memory_order_release); // 发布
        do_chunk(0); // master 算第 0 块
      ...
      void run(const float *w_, const float *x_, float *y_, int out_dim_, int in_dim_) {
        ...
        out_dim = out_dim_; in_dim = in_dim_; pair = false;
        publish_and_run();
      }
      ...
      void run_pair(const float *w1_, const float *w2_, const float *x_, float *y1_,
        ...
        out_dim = out_dim_; in_dim = in_dim_; pair = true;
        publish_and_run();
      }
    };
  ...
  void matvec_f32_neon_mt_kv(const float *w, const float *x, float *y, int out_dim,
    ...
    if (elems < kMinParallelElems || p.workers.empty()) {
      matvec_inline(w, x, y, out_dim, in_dim); // 小矩阵走单线程
      return;
    }
    p.run(w, x, y, out_dim, in_dim); // 大矩阵走多线程
  }
  ...
  void matvec_pair_f32_neon_mt_kv(const float *w1, const float *w2, const float *x,
    ...
    p.run_pair(w1, w2, x, y1, y2, out_dim, in_dim); // 合并 fork-join
  }
