// ============================================================================
// matvec_i4_sdot5.cpp — INT4 weight-only matvec：y = W @ x
//                  W4A8 SDOT v5（对称量化版，无 zero 修正项）
// ============================================================================
// 本文件实现两种变体：单线程 "sdot5" 和多线程 "sdot5_mt"。
//
// 相对 sdot4 的改动：**假设权重是对称量化的（zero=8）**，跳过
// - zero 点读取与修正项计算（C = A*(zero-8) = 0）
// - 激活前缀和（xqsum 修正项不再需要）
// - 每组省 5-6 条标量指令 + 每 token 省一次前缀和填表
//
// 预期适用于 --symmetric 导出的模型（zero 恒为 8）。对非对称模型（zero≠8）
// 数值不正确，但与 sdot4 对照可快速 A/B 对标量开销的贡献。
//
// 布局与 sdot4 完全一致（每组 [scale_fp16(2B) | zero_fp16(2B) |
// packed_uint4(G/2 B)]），但 zero 字段恒为 8（被内核忽略），不读取。
//
...
int8_t  g_xq5[kMaxInDim];            // int8 量化后的激活
...
float quantize_x_i8_v3(const float *x, int in_dim) {
    ...
    if (in_dim > kMaxInDim) {
        fprintf(stderr,
                "[matvec_i4_sdot5] fatal: in_dim=%d 超过 kMaxInDim=%d，"
                "请调大 kernels/matvec/matvec_i4_sdot5.cpp 的 kMaxInDim\n",
                in_dim, kMaxInDim);
        abort();
    }
    const int n = in_dim;
...
// ========================================================================
// dot_row_i4_sdot5() — 单行点积（内联组头 + 128 位解包）
// ========================================================================
// 参数：
//   row        — 该行权重起始地址（含组头）
//   in_dim     — 输入维度
...
//   scale_x    — 激活量化缩放因子
// 返回值：该行输出（fp32）
float dot_row_i4_sdot5(const uint8_t *row, int in_dim, int group_size, float scale_x) {
    ...
    for (int g = 0; g < gpr; ++g) {
        ...
        const float A = half_bits_to_float(sh) * scale_x;        // DOT 系数（对称量化：zero=8，C=0）
        ...
        for (; i < n32; i += 32) {
            ...
            int8x16_t xq0 = vld1q_s8(g_xq5 + col + i);
            int8x16_t xq1 = vld1q_s8(g_xq5 + col + i + 16);
        ...
        if (i + 16 <= group_elems) {
            ...
            int8x16_t xq = vld1q_s8(g_xq5 + col + i);
        ...
        for (; i < group_elems; ++i) {
            ...
            dot_g += (q - 8) * static_cast<int>(g_xq5[col + i]);
        }
        // ---- 浮点还原（对称量化：无 zero 修正项）----
        acc += A * static_cast<float>(dot_g);
        col += group_size;
    }
    return acc;
}
...
// ========================================================================
// dot_2rows_i4_sdot5() — 2 行并行点积：共享激活加载 + 独立链
// ========================================================================
// 与 sdot2 的 dot_2rows 同思路：两行各自一条 SDOT 链（ILP），
// 激活只加载一次。解包换成 128 位路径，组头内联读。
void dot_2rows_i4_sdot5(const uint8_t *row_a, const uint8_t *row_b,
    ...
    for (int g = 0; g < gpr; ++g) {
        ...
        for (; i < group_elems; ++i) {
            ...
            const int xq_v = static_cast<int>(g_xq5[col + i]);
            sum_a += (qa - 8) * xq_v;
            sum_b += (qb - 8) * xq_v;
        }
        // ---- 浮点还原（对称量化：无 zero 修正项）----
        acc_a += A_a * static_cast<float>(sum_a);
        acc_b += A_b * static_cast<float>(sum_b);
        col += group_size;
    }
    *out_a = acc_a;
    *out_b = acc_b;
...
// ========================================================================
// matvec_i4_sdot5() — 单线程入口
// ========================================================================
void matvec_i4_sdot5(const uint8_t *w, const float *x, float *y,
    ...
    int o = 0;
    for (; o + 1 < out_dim; o += 2) {
        dot_2rows_i4_sdot5(w + static_cast<size_t>(o) * row_bytes,
                           w + static_cast<size_t>(o + 1) * row_bytes,
                           in_dim, group_size, scale_x, &y[o], &y[o + 1]);
    }
    if (o < out_dim) {
        y[o] = dot_row_i4_sdot5(w + static_cast<size_t>(o) * row_bytes,
                                in_dim, group_size, scale_x);
    }
}

// ========================================================================
// 多线程版 sdot5_mt —— 动态行调度（work-stealing）
// ========================================================================

// 自旋等待（与 sdot2 相同：先 YIELD 再让出）
inline void spin_until_sdot5(const std::atomic<std::uint64_t> &a, std::uint64_t target) {
    int spins = 0;
...
// 静态切分下慢核拖尾的问题（见文件头 ①）。
inline int default_parallelism_sdot5() {
...
struct RowPoolSdot3 {
    ...
    RowPoolSdot3() {
        const int p = default_parallelism_sdot5();
    ...
    void worker_main(int) {
        ...
        for (;;) {
            spin_until_sdot5(job_gen, next_job);
    ...
    void do_work() {
        for (;;) {
            ...
            int o = begin;
            for (; o + 1 < end; o += 2) {
                dot_2rows_i4_sdot5(w + static_cast<size_t>(o) * row_stride,
                                   w + static_cast<size_t>(o + 1) * row_stride,
                                   in_dim, group_size, scale_x,
                                   &y[o], &y[o + 1]);
            }
            if (o < end) {
                y[o] = dot_row_i4_sdot5(w + static_cast<size_t>(o) * row_stride,
                                        in_dim, group_size, scale_x);
            }
        }
    }
    ...
    void run(const uint8_t *w_, float *y_, int out_dim_, int in_dim_,
        ...
        spin_until_sdot5(done_gen, expected_done);
    }
};

// Meyers singleton：进程内唯一常驻池
RowPoolSdot3 &pool_sdot5() {
...
// ========================================================================
// matvec_i4_sdot5_mt() — 多线程入口
// ========================================================================
void matvec_i4_sdot5_mt(const uint8_t *w, const float *x, float *y,
                        int out_dim, int in_dim, int group_size) {
    // 一次性量化激活
    const float scale_x = quantize_x_i8_v3(x, in_dim);
    ...
    pool_sdot5().run(w, y, out_dim, in_dim, group_size, scale_x);
}

} // namespace

// 自注册：单线程 "sdot5" + 多线程 "sdot5_mt"
TINYQWEN_MATVEC_I4_VARIANT(matvec_i4_sdot5, "sdot5");
TINYQWEN_MATVEC_I4_VARIANT(matvec_i4_sdot5_mt, "sdot5_mt");
