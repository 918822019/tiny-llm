// INT4 weight-only matvec：NEON + 常驻线程池行切分。
//
...
// 带宽（nibble unpack 吃满单核），多线程行切分把 unpack 摊到多核是本变体的
// 目的。group_size 运行时传入（v2 header quant_group_size），pool 按调用存储。
...
// 处理单行的一个 group（与 matvec_i4_neon 相同实现）。
...
inline float dot_row_i4(const uint8_t *row, const float *x, int in_dim, int group_size) {
    ...
    for (int g = 0; g < groups_per_row; ++g) {
        const uint8_t *gp = row + g * group_total_bytes;
...
// 端侧绑核场景由 bench 注入 TINYQWEN_MT_THREADS=绑核数。
...
// 常驻行切分线程池（结构同 f16 neon_mt_kv_nt 的 RowPool，单矩阵模式）。
struct RowPoolI4 {
...
// 粒度阈值（按元素数）：小矩阵单线程，省同步开销。与 f16 版同量级。
